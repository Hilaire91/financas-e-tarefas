...
// 1. App.tsx: Remove language switcher
let codeApp = fs.readFileSync('src/App.tsx', 'utf-8');
codeApp = codeApp.replace(/\{\/\* Language Switcher \*\/\}[\s\S]*?<\/select>/, '');
fs.writeFileSync('src/App.tsx', codeApp);

// 2. format.ts: Hardcode pt-BR
let codeFormat = `
...
`;
fs.writeFileSync('src/utils/format.ts', codeFormat.trim() + '\n');

// 3. i18n.ts: Only keep 'pt'
let codeI18n = `
...
export default i18n;
`;
fs.writeFileSync('src/i18n.ts', codeI18n.trim() + '\n');
